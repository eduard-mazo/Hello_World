...
long int x;
int temp,p,temp2,a,b,c,d,e,n,i,j,y,z,w,count,count2,min1,hor1,min2,hor2,ptr=0;
...
{cout<<"SELECCIONE EJERCICIOS "<<endl<<"-----------------"<<endl<<"#0  -- PROBLEMA #2"<<endl<<"#1  -- PROBLEMA #3"<<endl;
    ...
    cout<<"#12 -- PROBLEMA #15"<<endl<<"#13 -- PROBLEMA #17"<<endl;
    cin>>temp;
    ...
    case 13:
        ...
        while(!e)
        {
            p=flota;                     //Guardo numero a convertir
            flota=flota+1;              //comienzo con la ec triangular n*(n+1)/2
            flota/=2;
            flota=flota*p;
            j=flota;
            flota=p;                    //recupero variable inicial
            cout<<endl<<"NUMERO TRIANGULAR: "<<j<<endl;
            for(i=1;i<=j;i++)// FOR para obetener divisores
            {
                if(j%i==0)
                {
                    count++;// contador para chequear quien cumple con el objetivo
                    cout<<i<<" ";
                    if(count==n) e=1;
                }
            }
        ...
        cout<<endl<<"EL NUMERO TRIANGULAR CON MENOS DIVISORES QUE N ES: "<<j<<endl;
        break;
    ...
    case 12:
        ...
        cin>>dim; // dimension de la matriz
        k=dim;
        k/=2;
        if(k==0)
        {
